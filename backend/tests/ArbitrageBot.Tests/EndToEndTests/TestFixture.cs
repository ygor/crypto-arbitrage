...
public class TestFixture : IDisposable
{
    ...
    private RiskProfile SetupTestRiskProfile()
    {
        return new RiskProfile
        {
            ...
            MaxExposurePerTradingPair = 5000m,
            MaxTotalExposure = 10000m,
            MaxExecutionTimeMs = 3000,
            VerifyOpportunitiesBeforeExecution = true,
            MaxCapitalPerTradePercent = 10.0m,
            MaxCapitalPerAssetPercent = 25.0m,
            MaxSlippagePercentage = 0.2m,
            StopLossPercentage = 1.0m,
            MaxConcurrentTrades = 3,
            TradeCooldownMs = 1000,
            DailyLossLimitPercent = 5.0m,
    ...
    private void SetupPaperTradingServiceMock()
    {
        ...
        MockPaperTradingService.Setup(p => p.SimulateMarketBuyOrderAsync(
            ...
            .ReturnsAsync((string exchangeId, TradingPair tradingPair, decimal quantity, CancellationToken ct) =>
            {
                ...
                {
                    IsSuccess = true,
                    OrderId = $"paper-test-{Guid.NewGuid()}",
                    Timestamp = DateTimeOffset.UtcNow,
                    TradingPair = tradingPair,
                    TradeType = TradeType.Buy,
                    RequestedQuantity = quantity,
                    ExecutedQuantity = quantity,
                    ExecutedPrice = 10000m, // Fixed price for testing
                    TotalValue = quantity * 10000m,
                    Fee = quantity * 10000m * 0.001m, // 0.1% fee
